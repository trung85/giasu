 ...
 * @author vinnyalves
 */
class TutorPlugin_Model_Settings {
	...
	private function _validate( $key, $val, $die_on_error=true )
	{
		...
		{
			return $die_on_error ?
				   wp_die( __( sprintf( 'Invalid property %s for Settings Model', $key ), TutorPlugin::$instance->domain ) ) :
				   false;
		}
		...
		switch( $key )
		{
			case 'foo':
				...
				{
					return $die_on_error ?
						   wp_die( __( sprintf( 'Some message about %s', $key ), TutorPlugin::$instance->domain ) ) :
						   false;
				}
				break;
			default:
