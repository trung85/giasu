 ...
 * @author vinnyalves
*/
class TutorPlugin_Model_Ajax_Request {
	...
	private function _validate( $key, $val, $die_on_error=true )
	{
		...
		switch( $key )
		{
			case 'is_success':
				...
				{
					return $die_on_error ?
						   wp_die( __( sprintf( 'Ajax Request Model Property %s must be boolean', $key), $TutorPlugin->domain ) ) :
						   false;
				}
			case 'callback': // extremely basic check. a real JS identifier check would be too big
				...
				{
					return $die_on_error ?
						   wp_die( __( sprintf( 'Ajax Request Model Property %s must be a valid JS identifier', $key), $TutorPlugin->domain ) ) :
						   false;
				}
				break;
			case 'msg':
				...
				{
					return $die_on_error ?
						   wp_die( __( sprintf( 'Ajax Request Model Property %s must be an array, a string or null', $key), $TutorPlugin->domain ) ) :
						   false;
				}
				break;
			default:
